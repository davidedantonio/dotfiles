...
map("n", "<leader>ph", "<C-w>x", { desc = "Split Horizontally", noremap = true, silent = false })
...
map("n", "gd", ":lua vim.lsp.buf.definition()<cr>", { desc = "LSP Definition", noremap = true, silent = true })
map(
	"n",
	"<leader>lt",
...
map(
	...
	{ desc = "Open Neotree on current buffer", noremap = true, silent = false }
)
